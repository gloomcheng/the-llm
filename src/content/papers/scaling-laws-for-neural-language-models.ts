...
export const scalingLawsSources: SourceRecord[] = [
  {
    ...
    supports: [
      '確立神經語言模型交叉熵損失與參數量（N）、數據集大小（D）與浮點計算量（C）之間的精確冪律關係（Power Law）',
      '證明模型超參數（如深度、寬度、多頭注意力數量）在參數量固定時對測試損失影響極其微弱，規模（Scale）具有壓倒性支配力',
      '提出算力最佳分配處方：當計算預算增長時，應優先擴大模型參數量（N ∝ C^0.73），數據集規模僅需次要擴展（D ∝ C^0.27）',
      '發現臨界批次大小（Critical Batch Size）隨損失降低而呈冪律增長，允許更大規模的 GPU 平行加速而無邊際效益遞減',
      '確立小模型外推預測大模型性能的「風洞實驗方法」，將深度學習從盲目試誤轉變為可精確預測的工程科學',
    ],
    limits:
      '因實驗中採用固定的 10 萬步餘弦學習率排程與提前終止策略，低估了數據集規模擴展的邊際收益，該分配比例後於 2022 年被 DeepMind Chinchilla 修正為參數量與數據量等比例擴展（各佔 C^0.50）。',
  },
  {
    id: 'paper-2020-gpt3',
    kind: 'paper',
    ...
    limits:
      'GPT-3 驗證了參數量擴展的有效性，但因沿用 Kaplan 定律而僅使用了 3000 億 token 的數據集，以現代 Chinchilla 視角審視屬於嚴重訓練不足（under-trained）。',
  },
  {
    ...
    supports: [
      ...
      '提出計算最優法則（Chinchilla Optimal）：參數量 N 與訓練 token 數 D 應以 1:1 的比例對稱增長（各佔 C^0.50）',
...
export const scalingLawsPaper: PaperRecord = {
  ...
  abstractSummary:
    '我們研究了自回歸 Transformer 語言模型在跨越數個數量級的參數量、數據集大小與計算預算下的經驗縮放規律。實驗證實：交叉熵損失與模型參數量 N、數據集大小 D 以及計算量 C 呈現出精確的冪律（Power-law）關係，且在跨越 6 個數量級的範疇內未見任何飽和平頂。更重要的是，模型性能主要由規模三要素決定，而對網路深度、寬度、多頭數量等架構超參數極不敏感。當算力預算增加時，最佳配置是優先大幅擴展參數量，次要擴展數據量。這套物理規律為大規模模型訓練提供了精確的外推預測導航。',
  historicalQuestion:
  ...
  unknowns: [
    '原論文實驗中由於固定學習率排程與提前終止，數據集最佳擴展斜率被低估為 0.27，直到 2022 年 Chinchilla 實驗才被修正為 0.50。',
