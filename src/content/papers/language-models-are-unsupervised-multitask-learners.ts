...
export const gpt2Sources: SourceRecord[] = [
  {
    ...
    supports: [
      ...
      '將參數量規模擴展至 15 億（1.5B，GPT-1 的 10 倍以上），並構建 40GB 高品質網路文本數據集 WebText（800 萬篇 Reddit 高讚連結文檔）',
      '改進 Transformer 架構穩定性：改用 Pre-Layer Normalization（Pre-LN）、殘差層權重初始化縮放 1/√(2N)，並引入 Byte-level BPE 分詞器徹底根除未登錄詞（<unk>）',
  ...
  },
  {
    ...
    supports: [
      '對話式多輪問答數據集，被 GPT-2 作為檢驗 Zero-shot 閱讀理解能力的黃金基準',
      'GPT-2 在不使用該數據集 127,000 個問答訓練樣本的情況下，直接以閱讀文本與問答提示達到 55 F1 分數',
    ],
    limits:
...
export const gpt2Paper: PaperRecord = {
  ...
  abstractSummary:
    'QA、機器翻譯、閱讀理解與摘要等自然語言處理任務，向來依賴在特定任務數據集上進行有監督微調。本研究證明：當語言模型在包含數百萬網頁文檔的全新數據集 WebText 上訓練時，無需任何明確的參數微調，即可學會執行多種任務。當模型條件設定於文檔與提問時，在 CoQA 閱讀理解基準上獲得 55 F1，達到甚至超越三種專用有監督基準的表現。模型參數量對於零樣本任務轉移的成功至關重要，容量擴展可使跨任務表現呈現穩定的對數線性提升。這項 15 億參數的模型 GPT-2 展現出極富連貫性的文字生成能力，同時首次引發了關於強大生成模型潛在惡意用途與漸進式發布的廣泛關注。',
  historicalQuestion:
  ...
  motivationClaims: [
    {
      ...
      claim:
        '自然語言具有天然的任務封裝能力，多任務學習可在單純的自回歸語言模型中以條件概率 P(output | input, task) 自然實現。',
      explanation:
